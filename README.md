...
| 文件 | 说明 |
|------|------|
| `ai-generated-food-supplement.json` | 🍽️ 预设食物数据库，包含 584 种天然食物的营养数据，可直接导入应用 |
| `food-data-format.md` | 📝 食物数据导入格式说明，描述 JSON/CSV 文件的字段要求 |

> 💡 **使用方法**：应用已内置预设食物数据，在「设置 → 导入食物数据」页面点击「一键导入预设数据」即可导入 584 种天然食物。如需自定义数据，可按照 `food-data-format.md` 中的格式要求准备 JSON/CSV 文件后导入。
>
> ⚠️ **注意事项**：
> - 清空食物数据库不会影响已记录的饮食记录，记录已保存食物名称和营养数据的快照
> - 食物数据按最近添加时间排序，新添加的食物会显示在列表最前面
