...
25、现在，重写"添加自定义食物"这个页面。功能要求是：能够记录食物每百克的热量、碳水、蛋白质、脂肪；食物的名称；食物的图标；食物的单位（可选）；有单位的情况下（无单位设置时，为可选项），能够记录一个单位的重量/毫升，然后在记录时直接计算出该n个单位的四个营养数据的值；保存时直接保存进食物库。以上为该页面所有功能，不得有出入。能够跳转到该页面的按钮有：食物库中"自定义食物"页面右下角加号；"添加摄入"页面的“添加自定义食物”按钮；设置——食物管理——右下角加号，这三个按钮点击后，都跳转到"添加自定义食物"该页面。
