...
39、给你定义一下，食物库是我们存放所有食物的库（内置食物+自定义食物），设置中的"食物管理"页面、"食物库"页面--"最近添加"页面、"添加摄入"页面--搜索食物库。这些地方显示的都应该是我们所谓的食物库里的数据，我们要统一、确认一下这一点。相当于是食物库是底层，这些页面显示的是食物库里的食物
40、首页添加时，"添加摄入"页面中，搜索食物库数据时，用的也应该时食物库里的数据（图标、四大营养素等等食物数据）
41、数据生成器里的食物数据，应当是从食物库中选择并添加进记录的，而不是凭空生成，也就是说它用的应当是食物库里的数据（图标、四大营养素等等食物数据）
42、在"添加摄入"页面，点击"保存记录"后，应当直接跳转回首页
================================================================================
                              Git 版本控制操作指南
================================================================================
