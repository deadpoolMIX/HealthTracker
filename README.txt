...
59、"编辑睡眠记录"页面，应当支持跨天记录，比如是从1号晚上十一点睡到2号早晨七点这样，所以要加上能选择日期的功能。选择时和分的形式，不要用这种点击进去选择数字，而是能滑动转轮来选择时间（就像iPhone定闹钟的时候选择时间的方式）
60、首页日历界面的当日食物数据，排版要和首页的一样，并且目前的问题是食物的图标，日历页面食物的图标又和食物库不符了。总之你可以理解为，把首页显示的食物记录和日历中对应日期的食物记录部分同步，保持一致。
61、"添加饮食计划"页面，计划类型下面的部分"添加摄入"页面的排版、样式、功能一致（"添加摄入"页面的日期选择不需要），然后保留最下面的"保存计划"按钮
62、添加好的饮食计划，保存在饮食计划的数据库里，设置中的饮食计划删除，"计划"页面显示的就是该数据库里的所有计划，在"计划"页面右上角设置一个排序按钮，点开后，可以实现设置"计划"页面中计划按照名称的拼音首字母、添加计划的时间两种方式降序、升序排序，并且支持手动排序。
63、
================================================================================
                              Git 版本控制操作指南
================================================================================
