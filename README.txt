...
50、"添加摄入"页面的"添加自定义食物"不应该在框里面跟他们一块滚动，它在框外的下面接着。
51、首页食物图标现在是显示emoji了，但是不是食物库对应的emoji（比如，食物库中，牛奶的图标是牛奶的emoji，但是在首页显示牛奶的时候，图标就是别的emoji）请确保首页食物的图标、名称和食物库中对应的食物应当保持一致性
52、问题：修改窗口餐次选择中的餐次中，早餐显示完整，但是午餐晚餐和加餐现在再显示后，只能看到第一个字（也就是午、晚、加）。是不是字体太大，导致换行，然后选择框高度又有限制，导致换行到下面的第二个字看不见了？如果是，那就把字体调小一号；如果不是，那么餐次选择只需要等高，然后显示不下，可以左右滑动，你自行选择方案，只需要让我看到四个餐次的选项每个字都在同一行即可。
53、定义一件事，点击"最近摄入"和"自定义食物"的食物，进入修改页面时，这个页面其本质是"添加自定义食物"页面，只不过是把标题改为了"编辑食物"。所以刚才把"添加自定义食物"页面增加了每n克的那个功能，也应该同步到"编辑食物"的页面上。
54、
================================================================================
                              Git 版本控制操作指南
================================================================================
