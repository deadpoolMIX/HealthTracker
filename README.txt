...
63、睡眠记录的滚轮站的太大了，我不需要滚轮的样式，只要它是滚动调整时间就行了，然后入睡时间和起床时间两个区块太大了，而且之间没有间隔，导致底下的保存记录按钮都显示不全，也点击不了，把这个页面做的精简一点，然后一个页面能显示全部，不要往下滚才能显示全了
64、每次进入日历界面时，日历界面选择的日期应与首页当前显示的日期一致，也就是日历作用于首页，然后首页反过来作用于日历的意思（指日期这一块）。（举例，今日是7号，进入日历时，也是7号，然后我在日历里选择2号，回到首页显示的是2号，再回到日历里，也是我刚选择的2号）然后重开软件后，默认回到今天
65、"添加饮食计划"页面，选择星期这一行，要求能左右滑动（从右往左滑是是从周一到周日）然后能显示一周七天 （现在只能显示到周五）然后每个选项同高同宽
66、"添加饮食计划"页面搜索食物库下面的搜索框下面应该跟"添加摄入"页面的搜索框下面一样，有一个框里面能显示四行食物，然后这个框里面可以上下滑动，查看食物库里的食物，然后框下面有"添加自定义食物"的按钮，也就是说要和"添加摄入"页面的下半部分一样
67、"饮食计划"页面，点击计划，即可跳转到"编辑计划"页面，该页面应与"添加饮食计划"页面一模一样且同步，只是换个标题
68、"数据报表"页面，只保留周、月的筛选机制。
69、"营养素摄入"、"身体数据趋势"、"睡眠记录"三个区块点击都可以跳转到对应的一个页面。
70、先不要输出代码，先确认我的需求，并给我一个计划让我确定，现在我们来指定一下"营养素摄入"的子页面的内容。我要求有三个区块，分别是柱状图区块、平均摄入量区块、摄入总量区块；要求能按照周、月的筛选来观察数据；三个区块的显示数据的范围是同步的；平均摄入量区块和摄入总量区块要显示四个数据（热量、碳、蛋、脂）；柱状图在周的筛选下时，显示的是七天的每天的总数据，柱状图在月的筛选下时，显示的是四个周的每周的总数据。
71、先不要输出代码，先确认我的需求，并给我一个计划让我确定，现在我们来指定一下"身体数据趋势"的子页面的内容。我要求有两个区块，分别是折线图区块和数据变化总结区块，两个区块显示的数据的时间范围是同步且一致的；要求能手动选择时间范围（从哪年哪月哪日到哪年哪月哪日）；数据变化总结区块要求显示在该时间范围内：体重、体脂、肌肉的变化；然后该页面右上角点击可以切换体重体脂和三围，这是两个页面，但是功能一样。
72、先不要输出代码，先确认我的需求，并给我一个计划让我确定，现在我们来指定一下"睡眠记录"的子页面的内容。我要求有两个区块，分别是睡眠图表区块和睡眠总结区块，两个区块显示的数据的时间范围是同步且一致的；睡眠总结区块要求有：平均入睡时间点和平均起床时间点和平均睡眠时长；时间范围筛选有周、月；睡眠图表在周的筛选下，显示的是七天每天的，在月的筛选下，现实的是四周每周的平均；然后睡眠图表现在是横着的，我需要它是竖着的。
================================================================================
                              Git 版本控制操作指南
================================================================================
