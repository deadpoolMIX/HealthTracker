...
33、”添加摄入”页面中，选择食物库的食物后，弹出的窗口，应该是左边为输入框（输入数值），右边为选择框，点开以后，可以滑动、点击，选择对应的单位。并且计算出来对应的数值（要和食物库里的数据匹配，比如：食物库中食物a的100克为100kcal，在这个窗口中计算出来的也要一样，也就是说，用的是食物库里的数据）
34、问题：”食物管理”页面中，自定义添加的食物，在右侧变为了该食物的第一个汉字。（例如：我添加了一个叫”饮料”的食物，结果在右侧字母排序表中，出现了”饮”字）应该是这个食物第一个汉字的首字母（比如：”饮料”添加进去后，应该排在Y里面）
35、首页点击食物进行修改时，应当是弹出一个框，这个框里显示的是该食物当前的餐次、数值、单位，并且可以对这三项进行修改，修改后的结果同步到记录中。
================================================================================
                              Git 版本控制操作指南
================================================================================
