...
4、再添加第四个同层级的页面————饮食计划页面，该页面是我们添加的所有饮食计划.右下角有加号，点击即可添加饮食计划，饮食计划里可以是一天的内容（三餐）；也可以是一周的内容（每天每餐）；也可以是一顿的内容，添加饮食计划时的页面请与我商讨应该长什么养。
5、模拟并往软件里添加十天的摄入、睡眠、身体数据，模拟用户填写的方式，而不是内置进去，用作测试报表
6、首页顶部日期，点进去后应当进入一个页面，这个页面上半部分是日历形式，这个日历的每个日期的格子应当为热力图，也就是这个格子的背景颜色的深浅应当与当日摄入的热量多少相匹配；下半部分是上半部分日历选中的日期的当日摄入的饮食记录。
7、导入、导出备份数据，应当只包含文本数据，尽量精简。
